...
import { Search, History, LayoutGrid, ShieldCheck, UserCog, LogOut } from 'lucide-react'
...
export function Sidebar() {
  const { isAdmin, canManageRequests, isDoctor, logout } = useAuth()
  const navigate = useNavigate()

  // Doctors have full dataset access with no request-access workflow, so the
  // requests page/link isn't relevant to them.
  const navItems = isDoctor ? baseNavItems.filter((item) => item.to !== '/requests') : baseNavItems
